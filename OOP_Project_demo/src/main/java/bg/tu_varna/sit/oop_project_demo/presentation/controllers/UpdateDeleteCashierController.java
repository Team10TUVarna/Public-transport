...
public class UpdateDeleteCashierController implements Initializable {
    ...
    public void onDeleteCashierButtonClick(ActionEvent event) {
        CashierListViewModel selectedItem = cashierTable.getSelectionModel().getSelectedItem();
        cashierService.deleteCashier(selectedItem);
        initTable();
        Alert alert1=new Alert(Alert.AlertType.CONFIRMATION,"Successfully deleted cashier!",ButtonType.OK);
        alert1.show();
    }
    ...
    public void onUpdateCashierButtonClick(ActionEvent event) {
        CashierListViewModel selectedItem = cashierTable.getSelectionModel().getSelectedItem();
        if (cashierService.updateCashierHonorarium(selectedItem, Double.parseDouble(honorarium.getText()))){
            Alert alert1=new Alert(Alert.AlertType.CONFIRMATION,"Successfully updated cashier!",ButtonType.OK);
            alert1.show();
            initTable();
        }else{
            Alert alert1=new Alert(Alert.AlertType.ERROR,"Error when updating cashier!",ButtonType.OK);
            alert1.show();
        }
    }
